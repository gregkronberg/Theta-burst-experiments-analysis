...
dfiltHigh2 = designfilt('bandpassiir','FilterOrder',20,...
         'HalfPowerFrequency1',100,'HalfPowerFrequency2',2000, ...
         'SampleRate',fs);
                ...
                for e = 1:length(drug)
                    if isempty(slices{a,b,c,d,e}) == 0
                        ...
                        for f = 1:length(slices{a,b,c,d,e})
                            ...
                            indAll{a,b,c,d,e}(:,f) = indD;  % full induction traces (time x slices)
                            ...
                            baseSall = baseS; % store full baseline traces before cropping (time x blocks)
                            baseS = baseS((pulset+bipolarWidth):pulset+(fs/rBurst),:); % cropped baseline traces (time x slices)
                            
                             % load somatic recording during induction
                            indSall{a,b,c,d,e}(:,f) = indS; % full somatic recording during induction (time x slices)
