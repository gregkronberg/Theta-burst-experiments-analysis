    ...
    for i = 1:length(slicesR)                                              % loop over all slices
        if sum(strcmp(slicesP,slicesR{i}))==0                              % check if current slice is new
            ...
            for j = 1:size(comtext,1)                                      % loop through all comments
                ...
                if isempty(strfind(comtext(j,:),'induction2_')) == 0         % find plasticity induction block 2
                   indBlock(2) = com(com(:,5)==j,2);
                end
                if isempty(strfind(comtext(j,:),'induction3_')) == 0         % find plasticity induction block 3
                   indBlock(3) = com(com(:,5)==j,2);
                end
                if isempty(strfind(comtext(j,:),'induction4_')) == 0         % find plasticity induction block 4
